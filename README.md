...
微信/Open Graph 与 Twitter Card 描述使用两行英文：“Save the Date”与“Nov 20, 2026”，不再显示“诚邀您见证我们的婚礼”。

首屏视觉使用按展示尺寸重采样的 WebP；序言、相框和二十张相册图改为进入相应阶段前再加载。背景音乐在页面进入时立即发起本地完整音源的加载与播放，并在微信桥接器就绪或首次触摸时重试；浏览器或微信仍可能依据自己的自动播放策略拦截首次有声播放。`service-worker.js` 会缓存同源核心资源和已访问的后续资源，降低重复打开时对 GitHub Pages 边缘缓存与网络状态的依赖。

页面打开时先显示约 2 秒的深棕色 Loading 过渡：两个错位重叠的爱心使用白色与网站金色，并在 2 秒周期内交替换色。Loading 只等待首屏信封、印章、情侣插画和必要字体，2 秒后开始淡出，不等待完整背景音乐；脚本禁用时也会自动释放页面。
...
- Loading 不显示文字或情侣简笔画；先完整展示一次约 2 秒的白/金爱心换色周期，再用约 0.4 秒淡出。资源等待有 2 秒上限且不包含完整 BGM；减少动态和禁用 JavaScript 均不会卡死。
