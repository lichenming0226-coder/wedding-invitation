...
页面提供完整的 Open Graph、Twitter Card、`image_src` 与 canonical 元数据。分享封面是 `assets/share-preview-20260925.jpg`（1200×630 JPEG，公开绝对 HTTPS URL），供支持标准网页元数据的平台生成带图链接卡片。

首屏视觉使用按展示尺寸重采样的 WebP；序言、相框和二十张相册图改为进入相应阶段前再加载；跨域音乐只有在用户首次操作后才设置音源。`service-worker.js` 会缓存同源核心资源和已访问的后续资源，降低重复打开时对 GitHub Pages 边缘缓存与网络状态的依赖。
...
本仓库根目录的 `index.html`、`styles.css`、`app.js`、`fonts.css` 和 `assets/` 是当前维护源。GitHub Pages 从 `main` 根目录发布，`.nojekyll` 保留静态文件原样输出，无需构建。除按需加载的 Apple 官方音乐试听外，页面资源均为同源相对路径。
...
- 音乐不再参与首屏预加载；第一次轻触页面或点击音乐按钮后才加载并尝试启动。右上角按钮可暂停、继续，播放器状态与无障碍标签同步。
