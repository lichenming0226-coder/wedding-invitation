...
页面打开时先显示约 1–2 秒的深棕色 Loading 过渡：两个错位重叠的爱心使用象牙白与网站金色，并在 2 秒周期内交替换色。Loading 只等待首屏信封、印章、情侣插画和必要字体，最长等待 1.6 秒后开始淡出，不等待完整背景音乐；脚本禁用时也会自动释放页面。
...
- Loading 不显示文字或情侣简笔画；正常网络下约 1.75–1.88 秒完全淡出，关键图片延迟 1.5 秒时约 2.02 秒完全淡出。白/金爱心在 2 秒内完成颜色互换；减少动态和禁用 JavaScript 均不会卡死。
